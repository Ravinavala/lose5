	...
	var tmp, loading, overlay, wrap, outer, content, close, title, nav_left, nav_right,
		...
		_finish = function () {
			...
			if (currentOpts.type == 'iframe') {
				...
				$('<iframe id="fancybox_vgl-frame" name="fancybox_vgl-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + ($.browser.msie ? 'allowtransparency="true""' : '') + ' scrolling="' + selectedOpts.scrolling + '" src="' + currentOpts.href + '" allowfullscreen ></iframe>').appendTo(content).load(function() {
					if (selectedOpts && selectedOpts.showIframeLoading) {
