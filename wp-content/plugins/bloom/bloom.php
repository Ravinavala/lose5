...
<?php
	...
	private function _get_provider( $provider_slug, $account_name ) {
		return $this->providers->get( $this->provider_names[ $provider_slug ], $account_name, 'bloom' );
	}
	...
	public function _authorize_account() {
	...
	function authorize_account() {
		// Use new wrapper class if available
		$this->_authorize_account();

		// If we get this far then new wrapper class isn't available for the requested provider.
		// We already checked permissions and nonce.
		$service = ! empty( $_POST['bloom_upd_service'] ) ? sanitize_text_field( $_POST['bloom_upd_service'] ) : '';
		$name = ! empty( $_POST['bloom_upd_name'] ) ? sanitize_text_field( $_POST['bloom_upd_name'] ) : '';
		$update_existing = ! empty( $_POST['bloom_account_exists'] ) ? sanitize_text_field( $_POST['bloom_account_exists'] ) : '';

		if ( 'true' === $update_existing ) {
			$accounts_data = $this->_get_accounts();
	...
	function subscribe() {
		...
		// If we get this far then new wrapper class isn't available for the requested provider.
		// We already checked permissions, nonce and email.
		$subscribe_data_json = str_replace( '\\', '' ,  $_POST[ 'subscribe_data_array' ] );
		$subscribe_data_array = json_decode( $subscribe_data_json, true );
		...
		switch ( $service ) {
			case 'constant_contact' :
				$api_key       = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['api_key'] );
				$token         = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['token'] );
				$error_message = $this->subscribe_constant_contact( $email, $api_key, $token, $list_id, $name, $last_name );
				break;

			case 'madmimi' :
				$api_key       = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['api_key'] );
				$username      = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['username'] );
				$error_message = $this->subscribe_madmimi( $username, $api_key, $list_id, $email, $name, $last_name );
				break;

			case 'icontact' :
				$app_id        = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['client_id'] );
				$username      = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['username'] );
				$password      = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['password'] );
				$folder_id     = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['lists'][ $list_id ]['folder_id'] );
				$account_id    = sanitize_text_field( $all_accounts[ $service ][ $account_name ]['lists'][ $list_id ]['account_id'] );
				$error_message = $this->subscribe_icontact( $app_id, $username, $password, $folder_id, $account_id, $list_id, $email, $name, $last_name, $optin_id );
				break;
	...
	function subscribe_madmimi( $username, $api_key, $list_id, $email, $name = '', $last_name = '' ) {
		...
		if ( ! is_wp_error( $check_user_request ) && $check_user_response_code == 200 ){
			...
			if ( is_wp_error( $check_user_request ) ) {
				$error_message = $check_user_request->get_error_message();
			} else {
				switch ( $check_user_response_code ) {
					case '401' :
						$error_message = esc_html__( 'Invalid Username or API key', 'bloom' );
						break;
					default :
						$error_message = $check_user_response_code;
						break;
				}
			}
		}
	 ...
	 * @return string
	 */
	function subscribe_icontact( $app_id, $username, $password, $folder_id, $account_id, $list_id, $email, $name = '', $last_name = '', $optin_id = '' ) {
		...
		if ( is_array( $is_subscribed ) ) {
			...
			if ( '' !== $optin_id ) {
				$options_array = ET_Bloom::get_bloom_options();
				$current_optin = isset( $options_array[ $optin_id ] ) ? $options_array[ $optin_id ] : '';
				$message_id = isset( $current_optin['enable_dbl_optin'] ) && $current_optin['enable_dbl_optin'] && isset( $current_optin['message_id'] ) ? $current_optin['message_id'] : '';
			}

			$status = '' === $message_id ? 'normal' : 'pending';
			...
			if ( empty( $is_subscribed['contacts'] ) ) {
				...
				if ( '' !== $message_id ) {
					$add_body .= ',"confirmationMessageId":"' . $message_id . '"';
				}
				...
				if ( is_array( $added_account ) ) {
					if ( ! empty( $added_account['contacts'][0]['contactId'] ) ) {
						...
						if ( '' !== $message_id ) {
							$map_contact .= ',"confirmationMessageId":"' . $message_id . '"';
						}
	...
	function generate_new_account_form( $service, $account_name = '', $display_name = true ) {
		...
		$provider = $this->_get_provider( $service, '' );

		if ( $provider && 'aweber' !== $service ) {
			foreach ( $provider->get_account_fields() as $field_name => $field ) {
				$form_fields .= sprintf(
					'<div class="et_dashboard_account_row">
						<label for="%1$s">%2$s</label>
