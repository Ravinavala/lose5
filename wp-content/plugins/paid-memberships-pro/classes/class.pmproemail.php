...
//header and footer
    ...
    function sendCheckoutEmail($user = NULL, $invoice = NULL) {
        ...
        $groupcode = get_user_meta($user->ID, 'm_groupcode');
        if ($groupcode[0] != 0):
            $grpcode = $groupcode[0];
        else:
            $grpcode = '-';
        ...
        if (!empty($invoice) && !pmpro_isLevelFree($user->membership_level)) {
            ...
            $this->data["billing_address"] = pmpro_formatAddress($invoice->billing->name, $invoice->billing->street, "", //address 2
                    $invoice->billing->city, $invoice->billing->state, $invoice->billing->zip, $invoice->billing->country, $invoice->billing->phone);
