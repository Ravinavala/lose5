...
function ws_ls_get_weights($user_id, $limit = 100, $selected_week_number = -1, $sort_order = 'asc') {    // Check if data exists in cache.
    ...
    if (is_array($rows) && count($rows) > 0) {
        ...
        foreach ($rows as $raw_weight_data) {
            ...
            array_push($weight_data, ws_ls_weight_object($user_id, $raw_weight_data->weight_weight, $raw_weight_data->weight_pounds, $raw_weight_data->weight_stones, $raw_weight_data->weight_only_pounds, $raw_weight_data->weight_notes, $raw_weight_data->weight_date, false, $raw_weight_data->id, '', $measurements
            ));
        }
...
function ws_ls_get_weight($user_id, $row_id) {
    ...
    if (!is_null($row)) {
        ...
        return ws_ls_weight_object($user_id, $row->weight_weight, $row->weight_pounds, $row->weight_stones, $row->weight_only_pounds, $row->weight_notes, $row->weight_date, false, $row->id, '', $measurements, $row->photo_id
        );
    }
...
function ws_ls_save_data($user_id, $weight_object, $is_target_form = false, $existing_row_id = false) {
    ...
    if ($db_is_update != false) {
        $result = $wpdb->update(
                $table_name, $db_fields, array('id' => $db_is_update), $db_field_types, array('%d')
        );
    } else {
        $result = $wpdb->insert(
                $table_name, $db_fields, $db_field_types
        );
    }
...
function ws_ls_set_user_preferences($in_admin_area, $settings, $user_id = false, $height = false, $activity_level = false, $gender = false, $dob = false, $aim = false) {
    global $wpdb;
