<?php
global $gateway, $pmpro_review, $skip_account_fields, $pmpro_paypal_token, $wpdb, $current_user, $pmpro_msg, $pmpro_msgt, $pmpro_requirebilling, $pmpro_level, $pmpro_levels, $tospage, $pmpro_show_discount_code, $pmpro_currency, $pmpro_currency_symbol, $pmpro_currencies;
$pmpro_error_fields;
...
<div class="sign_up_sec min_height">
  ...
  <script>
        ...
        //debugger;
        var selectedpkg = $("input[name='selectedpkg']:checked").val();
        jQuery.ajax({
        url: '<?php echo admin_url('admin-ajax.php') ?>',
                type: "POST",
                data: {
                action: 'check_level_info',
          ...
          <div id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <div class="signup_row">
                            ...
                            <div id="displaygroupcode" style="display: none">
                                ...
                                <input id="groupcode" name="groupcode" type="text"  class="input <?php echo pmpro_getClassForField("groupcode"); ?>" placeholder="Enter Your Group Code" size="30" value="<?php echo esc_attr($groupcode) ?>"  maxlength="6" onblur="checkGroupcode()" />
...
$(document).on('click', '.pmpro_btn-submit-checkout', function (event) {
   ...
   if(firstname == '') {
        jQuery('#firstname').parent().find('span').after('<span class="error">First name is required.</span>');
        error = 1;
    } 
    else {
        jQuery('#firstname').parent().find('#error').remove();
    }

       if(lastname == '') {
        jQuery('#lastname').parent().find('span').after('<span class="error">Last name is required.</span>');
        error = 1;
    } 
    else {
        jQuery('#lastname').parent().find('#error').remove();
    }
