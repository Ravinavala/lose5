...
if (empty($current_user->membership_level))
    ...
    $confirmation_message = "<p>" . sprintf(__('Thank you for your to %s. Your %s membership is now active.', 'paid-memberships-pro'), get_bloginfo("name"), $current_user->membership_level->name) . "</p>";
$groupcode = get_user_meta($user_id, 'm_groupcode');

if ($current_user->membership_level == 'Team Caption'):
    $confirmation_message .= "<p>" . sprintf(__('You Registered as a', 'paid-memberships-pro'), $current_user->membership_level->name) . "</p>";
    $confirmation_message .= "<p>" . sprintf(__('You Groupcode is', 'paid-memberships-pro'), $groupcode) . "</p>";
endif;
...
<?php if (!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>		
    ...
    <ul>
        <li><strong><?php _e('Account', 'paid-memberships-pro'); ?>:</strong> <?php echo $current_user->display_name ?> (<?php echo $current_user->user_email ?>)</li>
        <li><strong><?php _e('Membership Level', 'paid-memberships-pro'); ?>:</strong> <?php
            if (!empty($current_user->membership_level))
                echo $current_user->membership_level->name;
            else
                _e("Pending", 'paid-memberships-pro');
            ?></li>
    </ul>	
    <?php
}
